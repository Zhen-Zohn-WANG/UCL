...
def main(device, args):
    ...
    for t in range(dataset.N_TASKS):
      ...
      # model_path = os.path.join(args.ckpt_dir, f"{args.name}_{datetime.now().strftime('%m%d%H%M%S')}_" + str(t) + '.pth')
      model_path = os.path.join(args.ckpt_dir, f"multitask_tinyimg_{args.name}_" + str(t) + '.pth')
      torch.save({
        'epoch': epoch+1,
